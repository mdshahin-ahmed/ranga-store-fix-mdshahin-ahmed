...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <div class="single-product">
      ...
      <button onclick="loadDetails(${product.id})" id="details-btn" class="btn">Details</button>
    </div>
      `;
...
const setDetails = (data) => {
  const singleProduct = document.getElementById('singelProduct');
  singleProduct.innerText = '';
  const div = document.createElement('div');
  div.innerHTML = `
    <img class="img-fluid mb-3" style="height: 14rem; width: 14rem;" src="${data.image}" alt="${data.category}">
    <h3>${data.title}</h3>
    <h5>Category: ${data.category}</h5>
    <h4>Price: $ ${data.price}</h4>
    <p>${data.description}</p>
  `;
  div.classList.add('details','shadow');
